...
final class SwitchEditor implements OptionEditor {
    public void render(ConfigOption option, GuiBounds bounds, GuiRenderer renderer, ConfigTheme theme,
        ...
        renderer.fill(x + 3, y + 3, x + width - 3, y + 4, enabled ? theme.accent : theme.border);
        renderer.fill(x + 3, y + 4, x + width - 3, y + 13, enabled ? theme.accentDark : theme.panelSunken);
        int knobX = enabled ? x + width - 13 : x + 4;
        renderer.fill(knobX, y + 4, knobX + 10, y + 13, theme.panelRaised);
        renderer.fill(knobX, y + 4, knobX + 10, y + 5, enabled ? theme.accent : theme.border);
        renderer.fill(knobX, y + 4, knobX + 1, y + 13, enabled ? theme.accent : theme.border);
