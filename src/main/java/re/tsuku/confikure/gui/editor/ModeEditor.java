...
final class ModeEditor implements OptionEditor {
    public void render(ConfigOption option, GuiBounds bounds, GuiRenderer renderer, ConfigTheme theme,
        ...
        EditorDraw.frame(renderer, x, y, width, 18, theme, false, false);
        ...
        if (leftHover) {
            ...
            renderer.fill(x + 1, y + 1, x + 20, y + 2, theme.border);
        }
        if (rightHover) {
            renderer.fill(x + width - 20, y + 1, x + width - 1, y + 17, theme.panelRaised);
            renderer.fill(x + width - 20, y + 1, x + width - 1, y + 2, theme.border);
        }
        EditorDraw.chevron(renderer, x + 8, y + 6, EditorDraw.Direction.LEFT,
                leftHover ? theme.accent : theme.mutedText);
        ...
        EditorDraw.chevron(renderer, x + width - 12, y + 6, EditorDraw.Direction.RIGHT,
                rightHover ? theme.accent : theme.mutedText);
    }
